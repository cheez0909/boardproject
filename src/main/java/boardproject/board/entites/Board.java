package boardproject.board.entites;

import boardproject.file.entites.FileInfo;
...
public class Board {
    ...
    private int listOrder; // 진열 가중치(가중치에 따라 순서를 바꿈)
    ...
    private MemberType listAccessType = MemberType.ALL; // 권한 설정 - 글목록

    @Enumerated(EnumType.STRING)
    @Column(length=20, nullable = false)
    private MemberType viewAccessType = MemberType.ALL; // 권한 설정 - 글보기

    @Enumerated(EnumType.STRING)
    @Column(length=20, nullable = false)
    private MemberType writeAccessType = MemberType.ALL; // 권한 설정 - 글쓰기

    @Enumerated(EnumType.STRING)
    @Column(length=20, nullable = false)
    private MemberType replyAccessType = MemberType.ALL; // 권한 설정 - 답글

    @Enumerated(EnumType.STRING)
    @Column(length=20, nullable = false)
    private MemberType commentAccessType = MemberType.ALL; // 권한 설정 - 댓글
